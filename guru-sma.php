...
[$menus, $permMap] = PermissionMiddleware::handle($pdo, 'guru-sma');

$service           = new GuruService($pdo);
$servicePermission = new PermissionService($pdo);

$canCreate = $servicePermission->can($permMap, 'guru-sma', 'create');
$canExport = $servicePermission->can($permMap, 'guru-sma', 'export');
$canImport = $servicePermission->can($permMap, 'guru-sma', 'import');
$canEdit   = $servicePermission->can($permMap, 'guru-sma', 'edit');
$canView   = $servicePermission->can($permMap, 'guru-sma', 'view');
...
if (isset($_GET['download']) && $canExport)
{
    ...
    header("Location: guru-export.php?jenjang=sma&{$q}");
    exit;
}

if (isset($_GET['upload']) && $canImport)
{
    header("Location: guru-import.php?jenjang=sma");
    exit;
}
...
try
{
    $result = $service->getGuru('guru_sma', $filter, $page, $limit);
}
catch (\Throwable $e)
{
